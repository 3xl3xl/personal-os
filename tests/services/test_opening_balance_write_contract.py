...
from personal_os.services import opening_balance_write_contract as writes
...
TRANSACTION_ID = uuid.UUID("00000000-0000-4000-8000-0000000000b1")
ACCOUNT_ID = uuid.UUID("00000000-0000-4000-8000-0000000000b2")
AUDIT_ID = uuid.UUID("00000000-0000-4000-8000-0000000000b3")


def request(**changes):
    values = dict(id=TRANSACTION_ID, account_id=ACCOUNT_ID,
                  amount=Money(500_000_00, "JPY"), occurred_at=NOW)
    values.update(changes)
    return AddOpeningBalanceInput(**values)
...
class FakeAccount:
    def __init__(self, currency_code="JPY"):
        self.currency_code = currency_code


class FakeUow:
    def __init__(self, failure=None, account=FakeAccount(), existing=()):
        self.events = []
        self.failure = failure
        self.staged_transaction, self.staged_audit = [], []
        self.saved_transaction, self.saved_audit = [], []
        self.accounts = SimpleNamespace(get=lambda _: account)
        self.transactions = SimpleNamespace(
            get=lambda _: None, add=self.add_transaction,
            list_by_account=lambda _: list(existing),
        )
    ...
    def add_transaction(self, record):
        ...
        self.staged_transaction.append(record)
    ...
    def commit(self):
        ...
        self.fail("commit")
        self.saved_transaction.extend(self.staged_transaction)
        self.saved_audit.extend(self.staged_audit)


def contract(uow, **kwargs):
    return writes.OpeningBalanceWriteContract(lambda: uow, clock=lambda: NOW, audit_id_factory=lambda: AUDIT_ID, **kwargs)
...
def test_success_order_full_audit(monkeypatch):
    ...
    result = contract(uow).add_opening_balance(request(), context=context())
    assert uow.events == ["permission", "enter", "transaction", "audit", "commit", "exit"]
    assert uow.saved_transaction == [result.transaction]
    assert result.transaction.status is TransactionStatus.ACTIVE
    assert result.transaction.transaction_type is TransactionKind.OPENING_BALANCE
    audit = uow.saved_audit[0]
    assert (audit.id, audit.actor, audit.action, audit.affected_entity_type) == (AUDIT_ID, "synthetic operator", "add_opening_balance", "transaction")
    assert audit.affected_entity_id == TRANSACTION_ID
    ...
    assert (audit.model_or_agent, audit.tool, audit.source) == ("synthetic-agent", "add_opening_balance", "synthetic-test")


def test_unapproved_stops_before_factory_or_clock():
    def forbidden():
        pytest.fail("unapproved write reached dependencies")
    service = writes.OpeningBalanceWriteContract(forbidden, clock=forbidden, audit_id_factory=forbidden)
    with pytest.raises(PermissionDeniedError):
        service.add_opening_balance(request(), context=context(approved=False))
...
def test_failures_leave_no_partial_fact_or_audit(phase):
    ...
    with pytest.raises(RuntimeError, match=phase):
        contract(uow).add_opening_balance(request(), context=context())
    assert uow.saved_transaction == uow.saved_audit == []
    assert uow.staged_transaction == uow.staged_audit == []
    assert "rollback" in uow.events


def test_duplicate_transaction_id_precedes_mutation():
    uow = FakeUow()
    uow.transactions.get = lambda _: object()
    with pytest.raises(writes.WriteReferenceError):
        contract(uow).add_opening_balance(request(), context=context())
    assert "transaction" not in uow.events and "commit" not in uow.events
    assert not uow.saved_transaction and not uow.saved_audit


def test_missing_account_precedes_mutation():
    uow = FakeUow(account=None)
    with pytest.raises(writes.WriteReferenceError):
        contract(uow).add_opening_balance(request(), context=context())
    assert "transaction" not in uow.events and "commit" not in uow.events


def test_currency_mismatch_precedes_mutation():
    uow = FakeUow(account=FakeAccount(currency_code="USD"))
    with pytest.raises(CurrencyMismatchError):
        contract(uow).add_opening_balance(request(), context=context())
    assert "transaction" not in uow.events and "commit" not in uow.events


def test_existing_opening_balance_rejected_precedes_mutation():
    existing = SimpleNamespace(transaction_type=TransactionKind.OPENING_BALANCE)
    uow = FakeUow(existing=(existing,))
    with pytest.raises(writes.WriteReferenceError, match="already has an opening balance"):
        contract(uow).add_opening_balance(request(), context=context())
    assert "transaction" not in uow.events and "commit" not in uow.events
    assert not uow.saved_transaction and not uow.saved_audit


def test_existing_normal_transaction_does_not_block_opening_balance():
    existing = SimpleNamespace(transaction_type=TransactionKind.NORMAL)
    uow = FakeUow(existing=(existing,))
    result = contract(uow).add_opening_balance(request(), context=context())
    assert uow.saved_transaction == [result.transaction]


@pytest.mark.parametrize("field", ["transaction_type", "status"])
...
def test_naive_time_rejected_and_offset_canonicalized():
    ...
    uow = FakeUow()
    result = contract(uow).add_opening_balance(request(occurred_at=offset), context=context())
    ...
    assert data["transaction_type"] == "OPENING_BALANCE"
...
def test_revalidate_copy_before_uow():
    uow = FakeUow()
    with pytest.raises(ValidationError):
        contract(uow).add_opening_balance(request().model_copy(update={"occurred_at": NOW.replace(tzinfo=None)}), context=context())
    assert not uow.events


def test_clock_failure_before_any_write():
    uow = FakeUow()
    service = writes.OpeningBalanceWriteContract(lambda: uow, clock=lambda: NOW.replace(tzinfo=None))
    with pytest.raises(ValueError):
        service.add_opening_balance(request(), context=context())
    assert not uow.events
...
def test_service_and_domain_have_no_orm_or_adapter_imports():
    ...
    for relative in ("services/opening_balance_write_contract.py", "domain/opening_balance_write_contracts.py"):
