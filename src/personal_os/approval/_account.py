"""Fail-closed macOS dialog approval for Account creation.
...
matching the same pattern already used for the batch-import dialog
...
_SCRIPT = '''on run argv
set answer to display dialog (item 1 of argv) with title "Personal OS — local account" buttons {"Cancel", "Approve once"} default button "Cancel" cancel button "Cancel" giving up after 120
...
def confirm_account(request: AddAccountInput, context: WriteContext) -> bool:
    ...
    details = json.dumps({
        "account": request.model_dump(mode="json"),
    ...
    if len(details) > 3500:
        return False
    prompt = ("Create ONE local Account record (no bank/broker link, no funds move).\n"
